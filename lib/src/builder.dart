...
const List<String> _kBlockTags = <String>['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'li', 'blockquote', 'pre', 'ol', 'ul', 'hr', 'table', 'thead', 'tbody', 'tr', 'cards'];
...
class MarkdownBuilder implements md.NodeVisitor {
  ...
  bool visitElementBefore(md.Element element) {
    ...
    if (_isBlockTag(tag)) {
      ...
      if (_isListTag(tag)) {
        ...
        // LAR: Added.
        if (length == 0) {
          decoration = styleSheet.tableHeadDecoration as BoxDecoration?;
        }
        // LAR: if (length == 0 || length.isOdd) {
        if (length.isOdd) {
          decoration = null;
        }
  ...
  void visitElementAfter(md.Element element) {
    ...
    if (_isBlockTag(tag)) {
      ...
      if (builders.containsKey(tag)) {
        ...
        final String? colorAttribute = element.attributes['color'];
        Color? color;
        if (colorAttribute != null) {
          var tryParse = int.tryParse(colorAttribute);
          color = (tryParse != null) ? Color(tryParse).withAlpha(255) : null;
        }
  ...
  Widget _buildImage(String src, String? title, String? alt, String? colorAttribute) {
    ...
    Color? color;
    if (colorAttribute != null) {
      final i = int.parse(colorAttribute.substring(1), radix: 16);
      color = Color(i).withAlpha(255);
    }

    final Uri uri = Uri.parse(path);
    Widget child;
    if (imageBuilder != null) {
      child = imageBuilder!(uri, title, alt, width, height, color);
    } else {
      child = kDefaultImageBuilder(uri, imageDirectory, width, height, color);
    }
  ...
  Widget _buildTableCell(List<Widget?> children, {TextAlign? textAlign, Color? color}) {
    // TODO: Added color
    var padding = Padding(
  ...
  void _addParentInlineIfNeeded(String? tag) {
    if (_inlines.isEmpty) {
      _inlines.add(_InlineElement(
        tag,
        // LAR: style: styleSheet.styles[tag!],
        style: tag != null ? styleSheet.styles[tag] ?? styleSheet.styles['p'] : styleSheet.styles['p'],
      ));
    }
  }
    ...
    for (final Widget child in children) {
      if (mergedTexts.isNotEmpty && mergedTexts.last is RichText && child is RichText) {
        ...
        final List<TextSpan> children = previousTextSpan.children != null
            ? previousTextSpan.children!.map((InlineSpan span) => span is! TextSpan ? TextSpan(children: <InlineSpan>[span]) : span).toList()
        ...
        final List<TextSpan> children = previousTextSpan.children != null ? List<TextSpan>.from(previousTextSpan.children!) : <TextSpan>[previousTextSpan];
