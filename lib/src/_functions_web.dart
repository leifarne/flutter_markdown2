...
typedef ImageBuilder = Widget Function(Uri uri, String? imageDirectory, double? width, double? height, Color? color);
...
final ImageBuilder kDefaultImageBuilder = (
  ...
  if (uri.scheme == 'http' || uri.scheme == 'https') {
    return Image.network(
      ...
      height: height,
      color: color,
      colorBlendMode: color != null ? BlendMode.plus : null,
    );
    ...
    final Uri fileUri = imageDirectory != null ? Uri.parse(p.join(imageDirectory, uri.toString())) : uri;
...
// ignore: prefer_function_declarations_over_variables
  ...
  switch (baseTheme) {
    case MarkdownStyleSheetBaseTheme.platform:
      ...
      result = userAgent.contains('Mac OS X') ? MarkdownStyleSheet.fromCupertinoTheme(CupertinoTheme.of(context)) : MarkdownStyleSheet.fromTheme(Theme.of(context));
      break;
